...
```{r}
filename <- "features/features_04_06.csv"
dataset_features_init <- read.csv(filename, header=TRUE)
dataset_features_init[dataset_features_init == "NaN"] <- NA
```
...
```{r}
sapply(dataset_features_init, class)
dataset_features_init$case_id <- as.factor(dataset_features_init$case_id)
dataset_features_init$tumor_mri_type <- as.factor(dataset_features_init$tumor_mri_type)
dataset_features_init$edema_mri_type <- as.factor(dataset_features_init$edema_mri_type)
```

## EXPLORATORY DATA ANALYSIS
* Primero se procede a escalar el dataset de las características.

```{r}
numeric_var_indexes_features <- c(which(sapply(dataset_features_init, class) == 'numeric', arr.ind = TRUE), which(sapply(dataset_features_init, class) == 'integer', arr.ind = TRUE))
non_numeric_var_indexes_features <- which(sapply(dataset_features_init, class) == 'factor', arr.ind = TRUE)

#preproc <- preProcess(dataset_features_init[,numeric_var_indexes_features], method=c("range"))
#dataset_features_scale <- predict(preproc, dataset_features_init[,numeric_var_indexes_features])
#dataset_features <- as.data.frame(cbind(dataset_features_scale, dataset_features_init[,non_numeric_var_indexes_features]))

dataset_features <- dataset_features_init
```
...
* De las cuales las primeras tres no llegan al 60% de datos completos y las otras cuatro sí. Se ha decidido el eliminar aquellas variables que no llegan al 60%, teniendo que imputar los valores de las variables number_of_pack_years_smoked en la fase de preprocesado. La variable years_high_alcohol_quantity_consumed ha sido eliminada pero se le da más importancia a number_of_pack_years_smoked teniendo en cuenta los resultados obtenidos en el análisis previo de los factores de riesgo.
...
* En la variable tumor_size hay 4 valores NA que tendrán que ser imputados.

* 3 de estas entradas NA corresponden a 3 pacientes de los cuales se tienen imágenes MRI de donde se han sacado características pero no se disponen metadatos de estos pacientes. Por ello, se decide eliminar estos 3 casos.
...
featurePlot(x=dataset_vital_status[, numeric_var_indexes_vital[17:20]],
            ...
            scales = list(y = list(relation="free"),
                          x = list(rot = 30)),
            layout = c(4, 1))
featurePlot(x=dataset_vital_status[, numeric_var_indexes_vital[21:25]],
            y=dataset_vital_status$vital_status_at_12months_follow_up, plot="box",
            scales = list(y = list(relation="free"),
                          x = list(rot = 30)),
            layout = c(5, 1))
...
* Analizando los boxplots y los barplots creados, no se puede determinar una separación clara entre las distintas variables de clase. La realidad es que hay ciertas tendencias, por ejemplo, parece que la mayoría de los que han muerto tenían un tamaño de tumor algo más alto como mediana. También se han observado ciertas variaciones en la distancia Y desde el centroide del cerebro al centroide del tumor y en la desviación estándar de los tumores. Pero no se observa que ninguna variable sea determinante para una predicción clara del estado del paciente. También parece que los que nunca han fumado tienden a sobrevivir.
...
featurePlot(x=dataset_cause[, numeric_var_indexes_cause[17:20]],
            ...
            scales = list(y = list(relation="free"),
                          x = list(rot = 30)),
            layout = c(4, 1))
featurePlot(x=dataset_cause[, numeric_var_indexes_cause[21:25]],
            y=dataset_cause$cause_of_death_at_12months_follow_up, plot="box",
            scales = list(y = list(relation="free"),
                          x = list(rot = 30)),
            layout = c(5, 1))
...
* En lo que respecta a los boxplots y barplots empleados, se observa que ciertas variables muestran una ligera diferencia entre clases. Si se ha observado que aquellos pacientes que nunca han fumado suelen morir por el GBM sin otras complicaciones. Otras variables como la desviación estándar del tumor, la distancia entre centroides en el eje Y, la compacidad del tumor o tumor_glcm_ASM también diferencian a pacientes con distintas causas de muerte pero esta diferencia no es muy significativa.
...
featurePlot(x = dataset_life_expect[, numeric_var_indexes_life_expect[17:20]], 
            ...
            span = 1,
            layout= c(2, 2))
featurePlot(x = dataset_life_expect[, numeric_var_indexes_life_expect[21:25]], 
            y = dataset_life_expect$life_expectancy,
            plot = "scatter",
            type = c("p", "smooth"),
            span = 1,
...
```
* Se puede ver que ciertas variables como el tamaño del tumor en cm o BMI muestran una relación más o menos lineal con la esperanza de vida. En otras, en cambio, esta relación no está tan clara.
...
* Podemos ver que el dataset imputado todavía contiene valores NA en las variables tumor_Hu_6, edema_Hu_3, edema_Hu_4, edema_Hu_5, edema_Hu_6, edema_Hu_7. Esto se debe a que el algoritmo ha detectado que estas variables son colineales con otras. Por lo tanto, se decide eliminar estas variables del dataset antes de terminar con la imputación.
```{r}
# Eliminar columnas que contienen NA
dataset_images_12month_features_completed <- dataset_images_12month_features_completed[ , colSums(is.na(dataset_images_12month_features_completed)) == 0]

# Cantidad de valores NA tras finalizar el proceso
print(sum(is.na(dataset_images_12month_features_completed)))

dataset_images_12month_features_completed$life_expectancy <- dataset_images_12month_features$life_expectancy
dataset_images_12month_features_completed$case_id <- dataset_images_12month_features$case_id

print(sum(is.na(dataset_images_12month_features_completed)))
```
* Se puede ver que tras eliminar las variables que presentaban una colinealidad, el dataset no tiene ningún valor NA. Los únicos valores NA presentes ahora son los de life_expectancy que corresponden a pacientes vivos.
...
Aun así, tras haber probado con todas las variables se excluye "tobacco_smoking_history" ya que da un coeficiente infinito y no converge. 
...
cox.model.all_vars <- coxph(Surv(time_monitored, vital_status_at_12months_follow_up) ~ bmi + number_of_pack_years_smoked + tumor_size_in_cm + tumor_volume + tumor_dist_x + tumor_dist_y + tumor_compactness + tumor_uf + tumor_kur + tumor_smoothness + tumor_glcm_contrast + tumor_glcm_energy + tumor_glcm_correlation + tumor_Hu_6 + edema_dist_x + edema_dist_y + edema_compactness + edema_uf + edema_skewness + edema_glcm_correlation + edema_glcm_ASM + edema_Hu_7 + age + height_in_cm + alcohol_consumption + gender, data = dataset_survival)
...
Mediante los tests ANOVA se ha determinado la relevancia de las variables en el modelo de Cox. La mayoría de ellos no parecen ser relevantes (p < 0.05), estando por debajo de ese valor únicamente las variables de alcohol_consumption, edema_dist_y, y edema_uf. Para asegurarnos de que esto se cumple, creamos un segundo modelo de Cox solo con las variables determinadas como relevantes y comparamos los resultados de los dos modelos mediante los tests ANOVA.

* Cabe destacar que tras realziar este análisis se ha visto que la diferencia entre ambos modelos era relevante por lo que se han tenido que incluir las siguientes tres variables tumor_glcm_correlation, tumor_glcm_contrast y bmi en este segundo modelo ya que son la que más cerca están de ser significativa.

```{r}
set.seed(173)

cox.model.select_vars <- coxph(Surv(time_monitored, vital_status_at_12months_follow_up) ~ alcohol_consumption + edema_dist_y  + edema_uf + tumor_glcm_correlation + tumor_glcm_contrast + bmi, data = dataset_survival)
...
El test ANOVA muestra un valor p > 0.05 por lo que podemos concluir que la diferencia entre ambos modelos no es relevante, confirmando que las seis variables seleccionadas son suficiente para calcular el modelo de supervivencia y que son las más determinantes a la hora de predecir las probabilidades de supervivencia del paciente que padece de GBM.

## RANDOM FOREST

Para comparar el modelo de supervivencia obtenido mediante la regresión de Cox con otro modelo se implementa otro modelo basádo en el algoritmo Random Forest.  Se calcula el modelo de supervivencia de todas las variables (incluyendo la variable "tobacco_smoking_history" excluida en la regresión de Cox) y obtenemos las importancias asignadas a cada una de las variables por el modelo mediante el método de permutación. Esta métrica nos permite determinar como aumenta el error en la predicción al eliminar la variable.
...
r_fit.selec_vars <- ranger(Surv(time = time_monitored, event = vital_status_at_12months_follow_up) ~  edema_dist_y + alcohol_consumption + edema_compactness + height_in_cm + tumor_hu_6 + tumor_glcm_energy + tumor_kur + tumor_compactness + edema_uf + gender + age, data = dataset_survival,
                           num.trees = 525,
                           importance = "permutation",
                           splitrule = "extratrees",
                           num.random.splits = 20,
...
var_imp
```
En el nuevo modelo se observa que las variables edema_uf, tumor_hu_6, tumor_compactness y age han dejado de ser relevantes, por lo que se puede concluir que las variables más determinantes a la hora de predecir la supervivencia del paciente son el historial de consumo de alcohol, la distancia entre el centroide del edema y el centroide del cerebro en el eje Y, la compacidad del edema, el género del paciente, la kurtosis del tumor y la altura del paciente.
