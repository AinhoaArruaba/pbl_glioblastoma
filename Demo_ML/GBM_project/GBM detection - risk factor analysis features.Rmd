...
#na_df <- dataset_features[rowSums(is.na(dataset_features)) > 0,]
#dataset_features <- dataset_features[complete.cases(dataset_features), ]
...
par(mfrow=c(1, 1))
correlation_numeric_vars_initial <- cor(dataset_images_merge_red[, c(numeric_var_indexes, life_expectancy_index)], use = "complete.obs")
...
featurePlot(x=dataset_vital_status[, numeric_var_indexes_vital[13:16]],
            ...
            scales = list(y = list(relation="free"),
                          x = list(rot = 30)),
            layout = c(4, 1))
featurePlot(x=dataset_vital_status[, numeric_var_indexes_vital[17:21]],
            y=dataset_vital_status$vital_status_at_12months_follow_up, plot="box",
            scales = list(y = list(relation="free"),
                          x = list(rot = 30)),
            layout = c(5, 1))
...
featurePlot(x=dataset_cause[, numeric_var_indexes_cause[13:16]],
            ...
            scales = list(y = list(relation="free"),
                          x = list(rot = 30)),
            layout = c(4, 1))
featurePlot(x=dataset_cause[, numeric_var_indexes_cause[17:21]],
            y=dataset_cause$cause_of_death_at_12months_follow_up, plot="box",
            scales = list(y = list(relation="free"),
                          x = list(rot = 30)),
            layout = c(5, 1))
...
featurePlot(x = dataset_life_expect[, numeric_var_indexes_life_expect[13:16]], 
            ...
            span = 1,
            layout= c(2, 2))
featurePlot(x = dataset_life_expect[, numeric_var_indexes_life_expect[17:21]], 
            y = dataset_life_expect$life_expectancy,
            plot = "scatter",
            type = c("p", "smooth"),
            span = 1,
...
Aun así, tras haber probado con todas las variables se excluyen "edema_glcm_correlation" y "edema_glcm_ASM" ya que dan un coeficiente infinito y no converge.
...
cox.model.all_vars <- coxph(Surv(time_monitored, vital_status_at_12months_follow_up) ~ age + gender + alcohol_consumption + bmi + tobacco_smoking_history + number_of_pack_years_smoked + height_in_cm + tumor_size_in_cm + tumor_dist_y + tumor_compactness + tumor_std + tumor_kur + tumor_smoothness + tumor_glcm_ASM + edema_kur, data = dataset_survival)
...
Mediante los tests ANOVA se ha determinado la relevancia de las variables en el modelo de Cox. La mayoría de ellos no parecen ser relevantes (p < 0.05), estando por debajo de ese valor únicamente las variables de BMI, kurtosis del edema y género (bmi, edema_kur y gender). Para asegurarnos de que esto se cumple, creamos un segundo modelo de Cox solo con las variables determinadas como relevantes y comparamos los resultados de los dos modelos mediante los tests ANOVA.

```{r}
set.seed(173)

cox.model.select_vars <- coxph(Surv(time_monitored, vital_status_at_12months_follow_up) ~ bmi + gender  + edema_kur, data = dataset_survival)
...
Para comparar el modelo de supervivencia obtenido mediante la regresión de Cox con otro modelo se implementa otro modelo basádo en el algoritmo Random Forest.  Se calcula el modelo de supervivencia de todas las variables (incluyendo las variables "edema_glcm_correlation" y "edema_glcm_ASM" excluidas en la regresión de Cox) y obtenemos las importancias asignadas a cada una de las variables por el modelo mediante el método de permutación. Esta métrica nos permite determinar como aumenta el error en la predicción al eliminar la variable.
...
r_fit.all_vars <- ranger(Surv(time = time_monitored, event = vital_status_at_12months_follow_up) ~ age + gender + alcohol_consumption + bmi + tobacco_smoking_history + number_of_pack_years_smoked + height_in_cm + tumor_size_in_cm + tumor_dist_y + tumor_compactness + tumor_std + tumor_kur + tumor_smoothness + tumor_glcm_asm + edema_kur + edema_glcm_correlation + edema_glcm_asm, data = dataset_survival,
                         num.trees = 525,
...
r_fit.selec_vars <- ranger(Surv(time = time_monitored, event = vital_status_at_12months_follow_up) ~  gender + tumor_size_in_cm + tobacco_smoking_history + height_in_cm + edema_glcm_correlation + tumor_glcm_asm + bmi + tumor_dist_y + tumor_smoothness + tumor_kur, data = dataset_survival,
                           num.trees = 525,
                           importance = "permutation",
                           splitrule = "extratrees",
                           num.random.splits = 20,
...
var_imp
```
En el nuevo modelo se observa que las variables de distancia entre centroides en el eje Y del tumor (tumor_dist_y) y la medida BMI (bmi) han dejado de ser relevantes, por lo que se puede concluir que las variables más determinantes a la hora de predecir la supervivencia del paciente son el historial de fumador de tabaco, el género, la altura del paciente, el tamaño del tumor en cm, la kurtosis del tumor, la correlación de la matriz de coocurrencia del edema, la suavidad del tumor y la métrica ASM de la matriz de coocurrencia del tumor.
