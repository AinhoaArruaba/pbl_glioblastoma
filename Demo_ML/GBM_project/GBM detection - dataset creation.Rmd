...
for(row in 1:dim(dataset)[1]){
  ...
  if(!is.na(dataset[row, ]$time_monitored) && dataset[row, ]$time_monitored > 365 * 3){
    dataset[row, ]$time_monitored <- 365
  }
}

dataset[,"life_expectancy"] <- NA
for(row in 1:dim(dataset)[1]){
  if(!is.na(dataset[row, ]$days_from_initial_pathologic_diagnosis_to_death_at_12months_follow_up)){
    dataset[row, ]$life_expectancy <- dataset[row, ]$days_from_initial_pathologic_diagnosis_to_death_at_12months_follow_up
  } else if(!is.na(dataset[row, ]$days_from_initial_pathologic_diagnosis_to_death_at_24months_follow_up)){
    dataset[row, ]$life_expectancy <- dataset[row, ]$days_from_initial_pathologic_diagnosis_to_death_at_24months_follow_up
  } else if(!is.na(dataset[row, ]$days_from_initial_pathologic_diagnosis_to_death_at_36months_follow_up)){
    dataset[row, ]$life_expectancy <- dataset[row, ]$days_from_initial_pathologic_diagnosis_to_death_at_36months_follow_up
  }else if(!is.na(dataset[row, ]$days_from_initial_pathologic_diagnosis_to_death_at_48months_follow_up)){
    dataset[row, ]$life_expectancy <- dataset[row, ]$days_from_initial_pathologic_diagnosis_to_death_at_48months_follow_up
  }
  
  if(!is.na(dataset[row, ]$life_expectancy) && dataset[row, ]$life_expectancy < 0){
    dataset[row, ]$life_expectancy <- dataset[row, ]$time_monitored
  }
}

for(row in 1:dim(dataset)[1]){
  if(!is.na(dataset[row, ]$life_expectancy) && !is.na(dataset[row, ]$time_monitored) && dataset[row, ]$life_expectancy > dataset[row, ]$time_monitored){
    dataset[row, ]$time_monitored <- dataset[row, ]$life_expectancy
  }
}
...
dataset_images_12month <- dataset_images_12month[, -which(names(dataset_images_12month) == 'days_from_initial_pathologic_diagnosis_to_death_at_12months_follow_up', arr.ind = TRUE)]
dataset_images_12month <- dataset_images_12month[, -which(names(dataset_images_12month) == 'days_from_initial_diagnosis_to_last_contact_at_12months_follow_up', arr.ind = TRUE)]
...
dataset_images_12month <- dataset_images_12month[, -which(names(dataset_images_12month) == 'tumor_status_at_12months_follow_up', arr.ind = TRUE)]
...
dataset_images_12month <- dataset_images_12month[, -which(names(dataset_images_12month) == 'tumor_site', arr.ind = TRUE)]
